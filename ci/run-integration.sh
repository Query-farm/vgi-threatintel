...
# vgi-threatintel VGI worker, using a prebuilt standalone `haybarn-unittest`
# and the signed community `vgi` extension — no C++ build from source.
# See ci/README.md.
#
# The threatintel worker enriches indicators against a reputation API over
# HTTP, so the suite needs a server: this script builds the repo's
# `mockserver`, starts it on a free port, and points the tests at it via
# VGI_THREATINTEL_TEST_URL (mirroring `make test-sql`).
#
# Required environment:
#   HAYBARN_UNITTEST        path to the haybarn-unittest binary
#   VGI_THREATINTEL_WORKER  worker LOCATION the .test files ATTACH (the built Go
...
# --- Start the mock reputation server (the .test files query it) ------------
...
# VGI_THREATINTEL_TEST_URL (base + /reputation, the base_url option), and kill
...
echo "Running suite (worker: $VGI_THREATINTEL_WORKER) ..."
